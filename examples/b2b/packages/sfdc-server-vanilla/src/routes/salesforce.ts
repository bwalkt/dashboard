...
export async function salesforceRoutes(fastify: FastifyInstance, options: FastifyPluginOptions): Promise<void> {
  ...
  // Delete all records by customer name
  // DELETE /salesforce/records/:objectType?customerName=John%20Doe
  // Query parameter:
  // - customerName: Customer name to match against Customer_Name__c field
  fastify.delete(
    "/salesforce/records",
    {
      preHandler: authenticateToken,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const { customerName } = request.query as { customerName?: string };

        if (!customerName) {
          reply.code(400).send({
            error: "Customer name is required",
            message: "Please provide customerName as a query parameter",
          });
          return;
        }

        // Query for all records matching the customer name
        // Using Customer_Name__c custom field to filter
        const soql = `SELECT Id FROM Order WHERE Customer_Name__c = null`;

        const queryResults = await salesforceClient.queryAll(soql);

        if (!queryResults.records || queryResults.records.length === 0) {
          reply.send({
            success: true,
            message: `No Order records found for customer: ${customerName}`,
            deletedCount: 0,
            recordIds: [],
          });
          return;
        ...
        const recordIds = queryResults.records.map((record: any) => record.Id).filter((id, index) => index > 0);
        const deletePromises = recordIds.map((recordId: string) => salesforceClient!.apiCall("DELETE", `/services/data/v58.0/sobjects/Order/${recordId}`));

        await Promise.all(deletePromises);

        reply.send({
          success: true,
          message: `Successfully deleted ${recordIds.length} Order record(s) for customer: ${customerName}`,
          deletedCount: recordIds.length,
          recordIds: recordIds,
        });
      } catch (error) {
        fastify.log.error(error, "Salesforce delete records by customer name error");
        reply.code(400).send({
