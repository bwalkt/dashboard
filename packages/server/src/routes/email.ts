...
export async function emailRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post<SendVerificationEmailRequest>(
    '/verify/email',
    async (request: FastifyRequest<SendVerificationEmailRequest>, reply: FastifyReply) => {
      try {
        ...
        await redis.set(redisKey, JSON.stringify({ email, name, createdAt: new Date().toISOString() }), 86400) // 24 hours
  ...
  fastify.post<SendVerificationEmailRequest>(
    ...
    async (request: FastifyRequest<SendVerificationEmailRequest>, reply: FastifyReply) => {
      try {
        ...
        // Store token in Redis with 24-hour expiration
        const redisKey = `email_verification:${verificationToken}`
        await redis.set(redisKey, JSON.stringify({ email, name, createdAt: new Date().toISOString() }), 86400)
  ...
  fastify.post<SendConfirmationCodeRequest>(
    ...
    async (request: FastifyRequest<SendConfirmationCodeRequest>, reply: FastifyReply) => {
      try {
   ...
   */
  fastify.get('/email/preview/slack-confirm', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
   ...
   */
  fastify.get('/email/preview/verification', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const html = await render(
        VerificationEmail({
          name: 'John Doe',
