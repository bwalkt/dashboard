...
SELECT
  pzero.create_user (
    jsonb_build_object (
      ...
      'john@example.com',
      'department',
      'Engineering', -- Goes into data
      'title',
      'Developer' -- Goes into data
    )
  );
...
SELECT
  pzero.create_user (
    jsonb_build_object (
      ...
      'jane@example.com',
      'data',
      jsonb_build_object ('department', 'Sales', 'title', 'Manager')
    )
  );
...
SELECT
  pzero.create_user (
    jsonb_build_object ('name', 'Bob Johnson', 'email', 'bob@example.com')
  );
```
...
const result = await db.query("SELECT pzero.create_user($1, $2, $3, $4, $5)", [
  name,
  email,
  orgId,
  part,
...
const result = await db.query("SELECT pzero.create_user($1)", [
  JSON.stringify({
    name: "John Doe",
    email: "john@example.com",
