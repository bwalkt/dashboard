...
CREATE DOMAIN PZERO.ID AS TEXT;
CREATE DOMAIN PZERO.IID AS BIGINT;
...
CREATE TABLE pzero.users (
  id pzero.IID NOT NULL UNIQUE REFERENCES pzero.auth(id) ON DELETE CASCADE,
  avatar TEXT,
...
CREATE INDEX idx_pzero_users_loc ON pzero.users using gin (location gin_trgm_ops);
...
CREATE TABLE pzero.active_sessions (
  id pzero.ID NOT NULLPRIMARY KEY,
  ip TEXT,
  user_agent TEXT
) INHERITS (pzero.LOC_BASE_TABLE);
...
CREATE INDEX idx_pzero_txns_c_by ON pzero.txns (c_by);
...
CREATE UNIQUE INDEX idx_pzero_relations_unique ON pzero.relations (UUID1, UUID2);
CREATE INDEX idx_pzero_relations_uuid1 ON pzero.relations (UUID1);
...
CREATE INDEX idx_pzero_audits_col ON pzero.audits (mmn,col_name);
...
DROP TABLE IF EXISTS pzero.col_defs;
