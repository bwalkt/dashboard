      ...
      {fields.map(field => {
        ...
        return (
          <div key={field.id.toString()}>
            {field.type === 'readonly' ? (
              <div className={cn('flex gap-4 my-1 py-1 text-sm justify-between items-center w-full', field.className)}>
              ...
              <DataTableSheetRowAction
                ...
                value={value}
                table={table}
                className={cn('flex gap-4 my-1 py-1 text-sm justify-between items-center w-full', field.className)}
              >
                <dt className="shrink-0 text-muted-foreground">{field.label}</dt>
                <dd className="font-mono w-full text-right">
                  {Component ? <Component {...data} metadata={metadata} /> : value}
