...
export function getFieldValueByType<TData>({ field, value }: { field?: DataTableFilterField<TData>; value: unknown }) {
  ...
  return {
    ...
    serialize: (columnFilters: ColumnFiltersState) => {
      const values = columnFilters.reduce((prev, curr) => {
        const { commandDisabled } = filterFields?.find(field => curr.id === field.value) || { commandDisabled: true } // if column filter is not found, disable the command by default
