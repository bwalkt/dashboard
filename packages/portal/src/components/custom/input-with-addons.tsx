...
const InputWithAddons = React.forwardRef<HTMLInputElement, InputWithAddonsProps>(
  ({ leading, trailing, containerClassName, className, ...props }, ref) => {
    return (
      ...
      >
        {leading ? <div className="border-input bg-muted/50 border-r px-3 py-2">{leading}</div> : null}
        <input
          className={cn(
          ...
          {...props}
        />
        {trailing ? <div className="border-input bg-muted/50 border-l px-3 py-2">{trailing}</div> : null}
      </div>
    )
  },
)
