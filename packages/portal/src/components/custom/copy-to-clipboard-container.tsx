...
const containerVariants = cva('peer whitespace-pre-wrap break-all rounded-md border p-2 font-mono text-sm', {
  variants: {
    variant: {
...
export const CopyToClipboardContainer = React.forwardRef<HTMLDivElement, CopyToClipboardContainerProps>(
      ...
      >
        ...
        {collapsible && !open ? (
          <div className="absolute inset-x-px bottom-px flex items-center justify-center rounded-b-md bg-gradient-to-b from-background/0 to-background/100">
            <Button variant="outline" size="sm" className="my-1 rounded-full" onClick={() => setOpen(true)}>
