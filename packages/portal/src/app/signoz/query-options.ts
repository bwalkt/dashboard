...
function transformTraceData(rawData: RawDataResponse["data"]): SignozTraceSchema[] {
  ...
  return rawData.map((item) => {
    ...
    if (item.responseStatusCode !== undefined && item.responseStatusCode !== null) {
      responseStatusCode = typeof item.responseStatusCode === "string" ? parseInt(item.responseStatusCode, 10) : item.responseStatusCode;
    }
 ...
 */
function transformTraceDataToSummary(rawData: any[]): { timestamp: number; success: number; warning: number; error: number }[] {
