// import { addDays, addMilliseconds, differenceInDays, differenceInMinutes, isSameDay } from 'date-fns'
// import { LEVELS } from '@/constants/levels'
// import { REGIONS } from '@/constants/region'
// import { isArrayOfDates, isArrayOfNumbers } from '@/lib/is-array'
// import { calculatePercentile, calculateSpecificPercentile } from '@/lib/request/percentile'
...
// export function filterData(data: ColumnSchema[], search: Partial<SearchParamsType>): ColumnSchema[] {
//   const { startTime, endTime, ...filters } = search
...
//           return false
//         } else if (filter.length === 2 && (row[key] < filter[0] || row[key] > filter[1])) {
//           return false
//         }
//         continue
//       }
...
//         } else if (
//           filter.length === 2 &&
//           (row[key].getTime() < filter[0].getTime() || row[key].getTime() > filter[1].getTime())
//         ) {
//           return false
//         }
//       }
...
//         }
//       }
//       if ((key === 'method' || key === 'host' || key === 'pathname') && Array.isArray(filter)) {
//         if (!filter.includes(row[key] as string)) {
//           return false
//         }
//         continue
//       }
//       if ((key === 'method' || key === 'host' || key === 'pathname') && typeof filter === 'string') {
//         if (row[key] !== filter) {
//           return false
//         }
//       }
...
//       if (item.date.getTime() < search.cursor.getTime() && newData.length < search.size) {
//         newData.push(item)
//         // TODO: check how to deal with the cases that there are some items left with the same date
//       } else if (item.date.getTime() === newData[newData.length - 1]?.date.getTime()) {
//         newData.push(item)
//       }
...
//           newData.push(item)
//         } else if (newData[newData.length - 1]?.date.getTime() === item.date.getTime()) {
//           newData.push(item)
//         }
//       }
//     }
