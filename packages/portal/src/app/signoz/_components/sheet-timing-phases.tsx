import { formatMilliseconds } from '@/lib/format'
import { getTimingColor, getTimingLabel, getTimingPercentage, TimingPhase, timingPhases } from '@/lib/request/timing'
import { cn } from '@/lib/utils'

export function SheetTimingPhases({
  latency,
  timing,
  className,
}: {
  latency: number
  timing: Partial<Record<TimingPhase, number>>
  className?: string
}) {
  const timingPercentage = getTimingPercentage(timing as Record<TimingPhase, number>, latency)
  return (
    <div className={cn('space-y-1 w-full text-left', className)}>
      {timingPhases.map(phase => {
        const value = timing[phase]
        const numValue = typeof value === 'number' ? value : 0
        return (
          <div key={phase} className="grid grid-cols-3 gap-2 text-xs justify-between items-center">
            <div className="text-foreground uppercase truncate font-mono">{getTimingLabel(phase)}</div>
            <div className="flex gap-2 col-span-2">
              <div className="font-mono text-muted-foreground mr-8">{timingPercentage[phase]}</div>
              <div className="flex flex-1 gap-2 items-center justify-end">
                <div className="font-mono">
                  {value === undefined || isNaN(numValue) ? (
                    <span className="text-muted-foreground">-</span>
                  ) : (
                    formatMilliseconds(numValue)
                  )}
                  {value !== undefined && !isNaN(numValue) ? <span className="text-muted-foreground">ms</span> : null}
                </div>
              </div>
              <div
                className={cn(getTimingColor(phase), 'h-4')}
                style={{ width: value === undefined || isNaN(numValue) || numValue === 0 ? 0 : `${(numValue / latency) * 100}%` }}
              />
            </div>
          </div>
        )
      })}
    </div>
  )
}
