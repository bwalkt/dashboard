...
export function PopoverPercentile({ data, percentiles, filterRows, className }: PopoverPercentileProps) {
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  active && data.percentile ? `border ${getPercentileColor(data.percentile).border}` : null,
                )}
              >
                <div className={cn('font-mono', !active && 'text-muted-foreground')}>{`P${Math.round(key)}`}</div>
