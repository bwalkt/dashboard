...
import { fetchDataTableData } from '@/lib/data-table-api'
...
export const dataOptions = (search: SearchParamsType) => {
  return infiniteQueryOptions({
    queryKey: ['data-table', searchParamsSerializer({ ...search, uuid: null, live: null })], // remove uuid/live as it would otherwise retrigger a fetch
    queryFn: async ({ pageParam }) => {
      ...
      // Use client-side data processing instead of API endpoint
      const searchParams = new URLSearchParams(serialize.substring(1)) // Remove leading '?'
      return await fetchDataTableData(searchParams)
    },
