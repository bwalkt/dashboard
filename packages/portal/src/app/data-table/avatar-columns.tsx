...
export const avatarColumns: ColumnDef<UserData>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="p-2 flex items-center justify-center h-full">
          <AvatarCell name={user.name} email={user.email} avatarUrl={user.avatarUrl} size="md" />
        </div>
      )
    },
    enableSorting: false,
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="p-2 flex items-center h-full">
          <Badge className={colorClass}>{department}</Badge>
        </div>
      )
    },
  ...
  {
    accessorKey: 'status',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Employee Status" />,
    cell: ({ row }) => {
