...
import { CHALLENGE_ANSWER_HEADER, CHALLENGE_ID_HEADER, CHALLENGE_PARAMS_HEADER, CHALLENGE_QUESTION_HEADER } from "../grid/challenge.js";
...
export const CHALLENGER_HEADERS = [CHALLENGE_ID_HEADER, CHALLENGE_HEADER, CHALLENGE_ANSWER_HEADER, CHALLENGE_PARAMS_HEADER];
/**
...
export function getChallengeSecret(): string {
  const secret = typeof window !== "undefined" ? (import.meta as any).env?.VITE_CHALLENGE_SECRET ?? "pzero" : process.env.VITE_CHALLENGE_SECRET ?? "pzero";
  return secret;
}
 ...
 */
export function serializeBody(body: any, headers: Record<string, string>): { body: any; headers: Record<string, string> } {
  if (body === undefined) {
...
export function isPublicPath(path: string): boolean {
  const publicPaths = ["/health", "/metrics", "/favicon.ico", "/robots.txt", "/sitemap.xml", "/auth/register", "/auth/login", "/auth/logout", "/auth/refresh"];
 ...
 */
export function createAuthHeaders(userId: string, email: string): Array<{ header: string; value: string; append: boolean }> {
