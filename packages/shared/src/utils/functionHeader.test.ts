...
describe('functionHeader utilities', () => {
  ...
  describe('prepareFunctionHeader', () => {
    ...
    it('should demonstrate significant space savings with compact grid format', async () => {
      const exprWithManyGridRefs =
        'add(grid[4][0], multiply(grid[1][2], grid[3][3], grid[2][1], grid[0][4], grid[5][2]))'

      const verboseResult = await prepareFunctionHeader(exprWithManyGridRefs, {
        useCompactGrid: false,
        useCompactFunctions: false,
      })
      const compactResult = await prepareFunctionHeader(exprWithManyGridRefs, {
        useCompactGrid: true,
        useCompactFunctions: false,
      })
