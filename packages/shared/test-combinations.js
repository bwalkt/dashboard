...
const grid = [[0, 0, 0], [0, 0, 31052], [0, 3901, 0]];
const func = {
...
const test1 = evaluate([[1000000, 0]], { expression: 'combinations(x, y)', xCell: { row: 0, col: 0 }, yCell: { row: 0, col: 1 } });
console.log('combinations(1000000, 0) =', test1, '(should be 1)');

// Test case where k=n (should be 1)  
const test2 = evaluate([[5000, 5000]], { expression: 'combinations(x, y)', xCell: { row: 0, col: 0 }, yCell: { row: 0, col: 1 } });
console.log('combinations(5000, 5000) =', test2, '(should be 1)');

// Test case with large n and small k (should compute correctly)
const test3 = evaluate([[10000, 3]], { expression: 'combinations(x, y)', xCell: { row: 0, col: 0 }, yCell: { row: 0, col: 1 } });
...
const test4 = evaluate([[2000, 1000]], { expression: 'combinations(x, y)', xCell: { row: 0, col: 0 }, yCell: { row: 0, col: 1 } });
