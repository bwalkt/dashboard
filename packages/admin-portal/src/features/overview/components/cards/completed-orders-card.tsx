...
/**
 * Display a dashboard card showing the count of completed orders from the last 30 days.
 *
 * Shows a loading state while orders are being fetched and an error state if the fetch fails.
 * When data is available the card displays the total number of orders with `Status === "Completed"`.
 * Clicking the card navigates the user to `/dashboard/orders`.
 *
 * @returns The rendered card element containing the completed orders summary and its loading/error states.
 */
    ...
    <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={handleCardClick}>
      <CardHeader>
        ...
        <CardTitle className="text-2xl font-semibold tabular-nums lg:text-3xl">
          {completedOrdersData.total.toLocaleString()}
        </CardTitle>
      </CardHeader>
      <CardFooter className="flex-col items-start gap-1.5 text-sm">
