...
func httpServerWithoutZiti(listenAddr string, router http.Handler) {
...
func zitifiedServer(router http.Handler) {
	...
	// Get service name (defaults to "http.svc" as in reference server)
	serviceName := "golang-proxy"
	if len(os.Args) > 2 {
...
func main() {
	...
	// Start Ziti server (if identity/config is provided via args)
	if len(os.Args) > 1 {
		go func() {
			defer func() {
				if r := recover(); r != nil {
		...
		log.Println("No Ziti identity file provided in args; skipping Ziti listener")
	}

	httpServerWithoutZiti("0.0.0.0:8080", router)
}
